...
@frappe.whitelist()
def get_op_opening_checklist_child_mobile(branch_param):
    print('get_op_opening_checklist_child_mobile =======', branch_param)
    print("-------- get data ------------")
...
@frappe.whitelist()
def get_op_opening_checklist_child_mobile_area(branch_param, area_param):
    print('get_op_opening_checklist_child_mobile =======', branch_param)
    print('get_op_opening_checklist_child_mobile =======', area_param)
    print("-------- get data ------------")
    current_date = datetime.today().date()
    build_sql = """
    SELECT
...
def update_op_opening_checklist_child_mobile(form_values):
    print('update_op_opening_checklist_child_mobile =======', form_values)
...
@frappe.whitelist()
def op_opening_check_today_record_exits_otherwise_create_one(branch,
                                                             current_date,
                                                             email_id,
                                                             user_name):
    parent_name = 0
    parent_name = op_opening_check_today_record_exist(branch, user_name, current_date)
    if (parent_name == 0):
...
def op_opening_check_today_record_exist(branch, user_name, current_date):
    print('-=-=-=-==-= op_opening_check_today_record_exist')
    print('b-id', branch, 'u-n', user_name, 'c-d', current_date)
    # current_date = datetime.today().date()
    ...
    if (rec_count > 0):
        print(" *** record_exists")
        paret_rec = frappe.get_last_doc('Op Opening Checklist', filters={
            'user_name': user_name,
            'branch': branch,
            'date': current_date
        })
        print(paret_rec)
        print(paret_rec.name)
        return paret_rec.name
...
def op_opening_insert_today_records(branch, current_date,
                                    email_id, user_name):
    print('op_opening_insert_today_records')
    print('branch-', branch)
