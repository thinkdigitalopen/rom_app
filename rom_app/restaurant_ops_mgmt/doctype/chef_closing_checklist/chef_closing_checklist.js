frappe.ui.form.on("Chef Closing Checklist", {
	...
	onload(frm) {
		...
		callback: function(res) {
			...
			frappe.call({
			method: "rom_app.restaurant_ops_mgmt.api.get_chef_closing_checklist_child",
			args: {branch_param: branch__id},
			callback: function(res_questions) {
				frm.doc.chef_close_questions = []

				$.each(res_questions.message, function(_i, e){
					let entry = frm.add_child("chef_close_questions");
					entry.chef_close_child_question = e[2];
				});
				refresh_field("chef_close_questions");
