frappe.pages['fb-close-check-mob'].on_page_load = function(wrapper) {
	...
	function save_form(){
		...
		console.log(values);
		let api_url = "rom_app.restaurant_ops_mgmt.api_mobile_fbc.update_fb_closing_checklist_child_mobile"
	...
	get_fb_closing_checklist_child();


	function get_fb_closing_checklist_child() {
		// window.location.reload(true);
		...
		get_fb_closing_checklist_child_mobile(basic_user_details);
	 }
	...
	function find_out_if_the_record_exist(user_details) {
		...
		let api_url = "rom_app.restaurant_ops_mgmt.api_mobile_fbc.fb_closing_check_today_record_exits_otherwise_create_one";
	...
	function get_fb_closing_checklist_child_mobile(basic_user_details){
		console.log("start - get_fb_closing_checklist_child_mobile");
		// 'parent_name': 4,
		...
		let api_url = 'rom_app.restaurant_ops_mgmt.api_mobile_fbc.get_fb_closing_checklist_child_mobile';
		...
		frappe.call({
			...
			callback: function(res_questions) {
				...
				console.log("frappe callback end - get_fb_closing_checklist_child_mobile");
			}
		});
	}
