...
class DmOpeningChecklistTemplate(Document):
    def before_insert(self):
        branch_selected = self.dm_open_template_branch
    ...
    def check_branch_already_exists(self, branch_id):
        result = frappe.db.get_list(
            "Dm Opening Checklist Template",
            filters={"dm_open_template_branch": branch_id},
            fields=["name", "dm_open_template_branch"],
            as_list=True,
        )
        return result
