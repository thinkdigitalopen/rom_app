frappe.pages['fb-close-check-mob'].on_page_load = function(wrapper) {
	...
	function load_items_for_the_date(load_date, basic_user_details) {
		...
		page.clear_fields();
		get_fb_closing_checklist_child(load_date, basic_user_details);
	}

	function get_fb_closing_checklist_child(load_date, basic_user_details) {
		console.log(" $$$$$$$$$ ");
		console.log(" load_date ", load_date);
	...
	function find_out_if_the_record_exist(load_date, user_details) {
		let api_url = "rom_app.restaurant_ops_mgmt.api_mobile_fbc.fb_closing_check_record_exits_otherwise_create_one";
		let ret_message = {};
		frappe.call({
			method: api_url,
			args: {
