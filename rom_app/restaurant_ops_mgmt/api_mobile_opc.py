...
#   Parent Id
    ...
    parent_doc = frappe.get_doc("Op Closing Checklist", field_parent_name)
    print(parent_doc)
...
def op_closing_insert_today_records(branch, current_date,
    ...
    q_template = api.get_op_closing_checklist_child(branch)
    print('q_template')
    print(q_template)
    ...
    for item in q_template:
        ...
        child = frappe.get_doc({
            'doctype': 'Op Closing Checklist Child',
            'parent': parent_new.name,
            'parentfield': 'questions',
            'parenttype': 'Op Closing Checklist',
            'audit': 0,
