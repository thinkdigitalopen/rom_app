...
class DmClosingChecklist(Document):
    ...
    def on_update(self):
        current_date = datetime.today().date()
        doc_save_date = datetime.strptime(self.dm_close_date, '%Y-%m-%d').date()
    ...
    def get_the_record_count(self, branch_id, user_name, date_obj):
        rec_count = frappe.db.count('Dm Closing Checklist', filters={
            'user_name': user_name,
            'branch_id': branch_id,
            'dm_close_date': date_obj
        })
        return rec_count
